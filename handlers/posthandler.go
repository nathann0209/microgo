...
func (p *Post) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	...
	fileExists := true
	if _, err := os.Stat("logs/requests.csv"); os.IsNotExist(err) {
		fileExists = false
	}

	// Open or create the CSV file in append mode
	file, err := os.OpenFile("logs/reqeust_metrics.csv", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
